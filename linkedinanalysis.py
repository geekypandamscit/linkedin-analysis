...
app.layout = dbc.Container([
    html.Hr(),
    dbc.Row(dbc.Col(html.H1("LinkedIn Analytics Dashboard",
    ...
    dbc.Row([
        dbc.Col([
            ...
            dbc.Card([
                dbc.CardBody([
                    dbc.CardLink("Geeky Panda", target="_blank",
...
# Bar Chart CONNECTION BY COMPANY ************************************************************
@app.callback(
...
def update_bar(start_date, end_date):
    ...
    fig_bar.update_yaxes(tickangle=50)
    fig_bar.update_layout(margin=dict(l=20, r=20, t=30, b=20))
    fig_bar.update_traces(marker_color='blue')'''
    #NEW DIAGRAM
    fig_bar = px.treemap(dff[:100], path=['Company', 'Position'], title="Total Connections by Company",
...
def update_pie(start_date, end_date):
    ...
    msg_sent = len(dff[dff['FROM']=='Parth Sarvaiya'])
    msg_rcvd = len(dff[dff['FROM'] != 'Parth Sarvaiya'])
